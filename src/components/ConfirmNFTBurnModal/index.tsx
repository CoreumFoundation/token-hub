...
import { setIsBurnNFTModalOpen, setIsConfirmNFTBurnModalOpen, setIsTxExecuting } from "@/features/general/generalSlice";
...
export const ConfirmNFTBurnModal = () => {
  ...
  const handleClose = useCallback(() => {
    dispatch(setIsConfirmNFTBurnModalOpen(false));
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      const txFee = await getTxFee([burnNFTMsg]);
      await signingClient?.signAndBroadcast(account, [burnNFTMsg], txFee ? txFee.fee : 'auto');
      ...
      <div className="flex flex-col w-full p-8 gap-8">
        <div className="flex flex-col text-center gap-2">
          ...
          <div className="font-noto-sans text-sm text-[#868991]">
            The NFT will be burned and removed from this collection. Would you like to proceed?
          </div>
        </div>
  ...
  return (
    <ConfirmationModal isOpen={isConfirmNFTBurnModalOpen}>
      <ConfirmationModalImage
        type={isTxSuccessful
          ? ConfirmationModalImageType.Success
          : ConfirmationModalImageType.Burn
        }
      />
      {renderContent}
    </ConfirmationModal>
